...
public static partial class CubeDetector
{
        ...
        out ModelPrimitive result)
    {
        ...
        return TryFitAndVerifyBox(faces, uniqueVertices, normals,
            directions[0], directions[1], directions[2], solid, out result);
    }
        ...
        out ModelPrimitive result)
    {
        ...
        foreach (Vector3 n in normals)
        {
            ...
            if (bestAlignment < 1f - NormalTolerance * 2f)
                return false;
        }

        return TryFitAndVerifyBox(faces, uniqueVertices, normals,
            ev0, ev1, ev2, solid, out result);
    }
        ...
        out ModelPrimitive result)
    {
        ...
        foreach (Vector3 v in uniqueVertices)
        {
            ...
            for (var axis = 0; axis < 3; axis++)
            {
                ...
                if (proj > halfExtent * (1f + RelaxedVertexTolerance))
                    return false;

                if (Mathf.Abs(proj - halfExtent) < halfExtent * RelaxedVertexTolerance + maxExtent * 1e-4f)
                    onSurface = true;
            }
        ...
        if (presentFaces < 3) return false;
        if (presentFaces >= 6) return false;

...
public class TriangulateV3Command : ICommand
{
    ...
    public string Command { get; } = "TriangulateV3";
    public string[] Aliases { get; } = [];
    public string Description { get; } = "Displays a model using hierarchical parenting (V3). Usage: <filename(.obj)> [accuracy(0.001)]";
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        {
            response = "Usage: TriangulateV3 <model file (.obj)> [accuracy(0.001)]";
            return false;
        }
    ...
    IEnumerator BuildRoutine(HierarchicalModel model, string fileName, int batchSize)
    {
        ...
        if (model.ParallelogramCount == 0)
        {
            model.Destroy();
            _model = null;
            Log.Warn($"[TriangulateV3] Model '{fileName}' has no valid triangles after async build.");
            yield break;
        }

        Log.Info($"[TriangulateV3] Created model '{fileName}': ParallelogramCount={model.ParallelogramCount}, PrimitiveCount={model.PrimitiveCount}, Reparented={model.ReparentedCount}, StretchesSaved={model.StretchesSaved}.");
    }
}